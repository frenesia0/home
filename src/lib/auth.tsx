...
> {
  try {
    return JSON.parse(
      localStorage.getItem(MOCK_REG_KEY) ?? '{}'
    );
  } catch {
    return {};
  }
...
export function mockMemberInfo(id: string): User | null {
  const hit =
    ...
    (isSetupDone() ? undefined : MOCK_ACCOUNTS[id]?.user);
...
export function AuthProvider({
  ...
  const loginWithGoogle =
    useCallback(async (): Promise<Result> => {
      ...
      /*
       * signInWithPopup 이후 onAuthChange도 user를 갱신하지만,
       * 여기서 한 번 더 읽어 즉시 화면에 반영한다.
       */
      try {
        ...
        /*
         * onAuthChange에서도 갱신되므로
         * 여기서 실패해도 로그인 자체는 유지된다.
         */
      }
  ...
  /*
   * 관리자가 로그인하면 body.admin
   * 페이지 설명 편집 연필 등에서 사용.
   *
   * 저장 계층이 훅 밖에서 작성자 id를
   * 알 수 있게 함께 기록한다.
   */
  useEffect(() => {
