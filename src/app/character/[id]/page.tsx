...
function ProfileArt({
  ...
  const mobileUrl = !bustFailed && bustUrl ? bustUrl : fullUrl;
  ...
  return (
    <>
      {fullUrl && (
        <img
          className={`char-art-full ${mobileFull ? 'mobile-show' : ''}`}
          src={fullUrl}
          alt={alt}
          draggable={false}
        />
      ...
      {!mobileFull && mobileUrl && (
        <div className="char-art-bust">
          {mobileUrl === bustUrl ? (
            ...
            <CropImg src={mobileUrl} crop={bustCrop} alt={alt} />
          )}
        </div>
      )}
    </>
